...
function tf_add_task_meta_boxes() {
	add_meta_box(
		'tf_task_details',
		__( 'Task Controls', 'timeflow-ai-1-2' ),
...
function tf_render_task_meta_box( $post ) {
	...
	<div class="tf-field-row">
		<label for="tf_project_id"><strong><?php esc_html_e( 'Project (required when creating under a project)', 'timeflow-ai-1-2' ); ?></strong></label>
		<select id="tf_project_id" name="tf_project_id">
			<option value="0"><?php esc_html_e( 'Standalone task (no project)', 'timeflow-ai-1-2' ); ?></option>
		...
		<p class="description tf-project-warning" <?php echo $project_id > 0 ? 'style="display:none;"' : ''; ?>>
			<?php esc_html_e( 'Warning: no project selected. This will be saved as a standalone task.', 'timeflow-ai-1-2' ); ?>
		</p>
	</div>

	<div class="tf-field-row">
		<label for="tf_task_status"><strong><?php esc_html_e( 'Status', 'timeflow-ai-1-2' ); ?></strong></label>
		<select id="tf_task_status" name="tf_task_status">
			<option value="pending" <?php selected( $status, 'pending' ); ?>><?php esc_html_e( 'Pending', 'timeflow-ai-1-2' ); ?></option>
			<option value="in_progress" <?php selected( $status, 'in_progress' ); ?>><?php esc_html_e( 'In Progress', 'timeflow-ai-1-2' ); ?></option>
			<option value="completed" <?php selected( $status, 'completed' ); ?>><?php esc_html_e( 'Completed', 'timeflow-ai-1-2' ); ?></option>
		</select>
	</div>

	<div class="tf-field-row">
		<p><strong><?php esc_html_e( 'Total Time Spent', 'timeflow-ai-1-2' ); ?>:</strong> <span id="tf-total-time-display"><?php echo esc_html( tf_format_duration( $total_time ) ); ?></span></p>
	</div>

	<div class="tf-field-row tf-controls" data-task-id="<?php echo esc_attr( $post->ID ); ?>" data-project-id="<?php echo esc_attr( $project_id ); ?>">
		<button type="button" class="button button-primary" id="tf-start-timer" <?php disabled( $is_active ); ?>><?php esc_html_e( 'Start Timer', 'timeflow-ai-1-2' ); ?></button>
		<button type="button" class="button" id="tf-stop-timer" <?php disabled( ! $is_active ); ?>><?php esc_html_e( 'Stop Timer', 'timeflow-ai-1-2' ); ?></button>
	</div>

	<div class="tf-field-row tf-controls" data-task-id="<?php echo esc_attr( $post->ID ); ?>">
		<label for="tf-manual-minutes"><strong><?php esc_html_e( 'Add Time (minutes)', 'timeflow-ai-1-2' ); ?></strong></label>
		<input type="number" id="tf-manual-minutes" min="1" step="1" />
		<button type="button" class="button" id="tf-add-time"><?php esc_html_e( 'Add Time', 'timeflow-ai-1-2' ); ?></button>
	</div>

	<div id="tf-message" class="tf-message" aria-live="polite"></div>

	<h4><?php esc_html_e( 'Time Logs', 'timeflow-ai-1-2' ); ?></h4>
	<table class="widefat striped tf-log-table">
		<thead>
		<tr>
			<th><?php esc_html_e( 'Type', 'timeflow-ai-1-2' ); ?></th>
			<th><?php esc_html_e( 'Project', 'timeflow-ai-1-2' ); ?></th>
			<th><?php esc_html_e( 'Start', 'timeflow-ai-1-2' ); ?></th>
			<th><?php esc_html_e( 'End', 'timeflow-ai-1-2' ); ?></th>
			<th><?php esc_html_e( 'Duration', 'timeflow-ai-1-2' ); ?></th>
		</tr>
		</thead>
		<tbody id="tf-log-body">
		<?php if ( empty( $logs ) ) : ?>
			<tr><td colspan="5"><?php esc_html_e( 'No logs yet.', 'timeflow-ai-1-2' ); ?></td></tr>
		<?php else : ?>
			<?php foreach ( array_reverse( $logs ) as $log ) : ?>
				<tr>
					<td><?php echo esc_html( ucfirst( $log['type'] ) ); ?></td>
					<td><?php echo esc_html( (int) $log['project_id'] > 0 ? get_the_title( (int) $log['project_id'] ) : __( 'Standalone', 'timeflow-ai-1-2' ) ); ?></td>
