...
function tf_start_timer( $task_id ) {
	...
	if ( $task_id <= 0 ) {
		return new WP_Error( 'tf_invalid_selection', __( 'Please select a project and task', 'timeflow-ai-1-2' ) );
	}

	if ( $active_task > 0 && $active_start > 0 ) {
		return new WP_Error( 'tf_timer_exists', __( 'Timer already running', 'timeflow-ai-1-2' ) );
	}

	$task = get_post( $task_id );
	if ( ! $task || 'tf_task' !== $task->post_type ) {
		return new WP_Error( 'tf_invalid_task', __( 'Invalid task.', 'timeflow-ai-1-2' ) );
	}
...
function tf_stop_timer() {
	...
	if ( $task_id <= 0 || $start_time <= 0 ) {
		return new WP_Error( 'tf_no_timer', __( 'No active timer is running.', 'timeflow-ai-1-2' ) );
	}
...
function tf_add_manual_time( $task_id, $minutes ) {
	...
	if ( $minutes <= 0 ) {
		return new WP_Error( 'tf_invalid_minutes', __( 'Minutes must be greater than zero.', 'timeflow-ai-1-2' ) );
	}
