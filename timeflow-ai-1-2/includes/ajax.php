...
function tf_ajax_validate_request() {
	...
	if ( ! current_user_can( 'edit_posts' ) ) {
		wp_send_json_error( array( 'message' => __( 'Permission denied.', 'timeflow-ai-1-2' ) ), 403 );
	}
}
...
function tf_ajax_start_timer() {
	...
	wp_send_json_success(
		array(
			'message' => __( 'Timer started.', 'timeflow-ai-1-2' ),
			'data'    => $result,
		)
	);
}
...
function tf_ajax_stop_timer() {
	...
	wp_send_json_success(
		array(
			'message' => __( 'Timer stopped.', 'timeflow-ai-1-2' ),
			'data'    => $result,
		)
	);
}
...
function tf_ajax_add_manual_time() {
	...
	wp_send_json_success(
		array(
			'message' => __( 'Manual time added.', 'timeflow-ai-1-2' ),
			'data'    => $result,
		)
	);
}
