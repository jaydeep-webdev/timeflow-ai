	...
	return array_map(
		static function ( $task ) {
			...
			return array(
				...
				'project'    => $project_id > 0 ? get_the_title( $project_id ) : __( 'Standalone', 'timeflow-ai-1-2' ),
...
function tf_dashboard_data() {
	...
	foreach ( $logs as $log ) {
		...
		$log_rows[] = array(
			'project'  => $project_id > 0 ? get_the_title( $project_id ) : __( 'Standalone', 'timeflow-ai-1-2' ),
			'task'     => $task_id > 0 ? get_the_title( $task_id ) : __( 'Unknown', 'timeflow-ai-1-2' ),
			'duration' => $duration,
...
function tf_handle_dashboard_action( $request ) {
	...
	return new WP_REST_Response( array( 'success' => false, 'message' => __( 'Invalid action.', 'timeflow-ai-1-2' ) ), 400 );
}
